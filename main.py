...
# =========================================================
# RIObot GOLD — XAUUSD M1 LIVE PSAR 1st DOT
# MAX 1 trade
# TP +8.00 | SL -10.00 | BE +7.00 -> lock +5.00
# TP/SL/BE are calculated from REAL MT5 openPrice.
# =========================================================
...
async def get_current_m1_candle(region):
    ...
    def fetch():
        ...
        if r.status_code != 200:
            raise RuntimeError(
                f"M1 candle HTTP {r.status_code}: {r.text[:250]}"
            )
...
# =========================================================
# EXACT SL / TP FROM REAL openPrice
# =========================================================

async def ensure_exact_stops(connection):
    positions = await get_positions(
        connection
    )
...
async def open_trade(
    ...
    # Po otvorení načítame reálnu openPrice.
    ...
    if position is None:
        print(
            ...
            f"{side} "
            f"{SYMBOL}; "
            f"waiting for MT5 position sync",
            flush=True,
        )
