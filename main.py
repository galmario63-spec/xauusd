...
FIB_MIN = 0.50
FIB_MAX = 0.618
RISK_REWARD_RATIO = 3.0  # Väčší cieľ pre TP
LOT_SIZE = 0.01
...
async def main():
    ...
    while True:
        try:
            # Opravené sťahovanie historických sviečok cez správne MetaApi rozhranie
            end_time = datetime.utcnow()
            start_time = end_time - timedelta(hours=5)
            
            candles = await connection.get_historical_candles(SYMBOL, TIMEFRAME, start_time, end_time)
